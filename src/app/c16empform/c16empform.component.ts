...
@Component({
  selector: 'app-c16empform',
  templateUrl: './c16empform.component.html',
  styleUrls: ['./c16empform.component.css']
})
export class C16empformComponent implements OnInit {

  empno:string;
  constructor(private activateRoute:ActivatedRoute,private appService : AppService,private router:Router)
   {
    this.empno=this.activateRoute.snapshot.queryParams['id'];
  ...
  empFormOnSubmit()
  {
    ...
    if(this.btnClicked=='delete')
    {
      ...
      console.log(empno);
      // console.log(emp);
      this.appService.deleteEmp(empno).subscribe(data=>{
        console.log(data);
        this.showSuccess("Emp Successfully delete","Success");
      },error=>{
        console.log('error',error);
      });
    }
